...
| `withdrawBucket(epoch, bucket, to)` | owner | `bucket ∈ {Acquisition, Contingency}`, `to != 0`, monto disponible. Gasto operativo, fuera de escrow. |
...
- `withdrawBucket` rechaza los buckets de origen y crawls.
