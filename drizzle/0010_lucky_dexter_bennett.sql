...
ALTER TABLE "consumption_proof" ADD COLUMN "yape_ref" text NOT NULL;--> statement-breakpoint
...
ALTER TABLE "consumption_proof" ADD CONSTRAINT "consumption_proof_submitted_binding" CHECK ("consumption_proof"."status" NOT IN ('submitted', 'confirmed') OR ("consumption_proof"."consumer_user_id" IS NOT NULL AND "consumption_proof"."purchase_order_id" IS NOT NULL));