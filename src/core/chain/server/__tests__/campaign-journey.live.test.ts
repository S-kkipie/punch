...
import { recoverStuckJobs, runRelayerOnce } from "@/core/chain/server/relayer/relayer";
...
const live = process.env.PUNCH_RUN_INTEGRATION === "1" && process.env.PUNCH_RUN_LIVE_CHAIN === "1";
...
async function findFixture() {
    const [consumer] = await db.select({ id: user.id, walletAddress: user.walletAddress }).from(user).where(eq(user.email, "demo-consumer@punch.pe"));
    const [owner] = await db.select({ id: user.id, walletAddress: user.walletAddress }).from(user).innerJoin(cafeMember, eq(cafeMember.userId, user.id)).where(and(eq(cafeMember.role, "owner"), eq(user.email, "esquinasur@punch.pe")));
    const [barista] = await db.select({ id: user.id }).from(user).where(eq(user.email, "esquinasur@punch.pe"));
    const [targetCafe] = await db.select({ id: cafe.id }).from(cafe).where(eq(cafe.slug, "esquina-sur"));
    if (!consumer?.walletAddress || !owner?.walletAddress || !barista || !targetCafe) throw new Error("campaign journey seed precondition is missing");
    const [product] = await db.select({ id: cafeProduct.id }).from(cafeProduct).where(and(eq(cafeProduct.cafeId, targetCafe.id), eq(cafeProduct.type, "emission"))).limit(1);
...
async function createPurchase(label: string) {
    const issued = await createPurchaseProofService(baristaId, cafeId, { productId, yapeRef: `${yapePrefix}-${label}` });
    expect(issued.ok, issued.ok ? undefined : JSON.stringify(issued.error)).toBe(true);
    ...
    if (!confirmed.ok) throw new Error("campaign journey quote confirmation failed");
...
describeLive("live campaign journey and chain projections", () => {
    beforeAll(async () => {
        ...
        expect(created.ok, created.ok ? undefined : JSON.stringify(created.error)).toBe(true);
    ...
    it("creates, funds, and publishes exact campaign terms", async () => {
        const [created] = await db.select().from(campaign).where(eq(campaign.id, campaignId));
        expect(created?.chainCampaignId).toEqual(expect.any(Number));
        chainCampaignId = created?.chainCampaignId ?? 0;
        const funded = await fundCampaignService(ownerId, cafeId, campaignId, budget);
        expect(funded.ok, funded.ok ? undefined : JSON.stringify(funded.error)).toBe(true);
        await drainRelayerAndIndexer();
        const published = await publishCampaignService(ownerId, cafeId, campaignId);
        expect(published.ok, published.ok ? undefined : JSON.stringify(published.error)).toBe(true);
        await drainRelayerAndIndexer();
        const [projection] = await db.select().from(projectionCampaign).where(eq(projectionCampaign.chainCampaignId, chainCampaignId));
        expect(projection).toMatchObject({ status: "published", voucherPayout: payout, maxVouchers: cap, budget });
    });
    ...
    it("projects a qualifying voucher only after VoucherUnlocked is confirmed", async () => {
        ...
        await runRelayerOnce();
        await runIndexerOnce();
        expect(await db.select({ id: consumerVoucher.id }).from(consumerVoucher).where(eq(consumerVoucher.campaignId, campaignId))).toHaveLength(0);
        await runRelayerOnce();
        await runIndexerOnce();
        const [voucher] = await db.select({ id: consumerVoucher.id }).from(consumerVoucher).where(eq(consumerVoucher.campaignId, campaignId));
        expect(voucher).toBeDefined();
        voucherId = voucher?.id ?? "";
        const [projection] = await db.select({ unlockedCount: projectionCampaign.unlockedCount }).from(projectionCampaign).where(eq(projectionCampaign.chainCampaignId, chainCampaignId));
    ...
    it("converges a repeated same-consumer café purchase without a second voucher", async () => {
        ...
        await drainRelayerAndIndexer();
        expect(await db.select({ id: consumerVoucher.id }).from(consumerVoucher).where(eq(consumerVoucher.campaignId, campaignId))).toHaveLength(1);
        const [projection] = await db.select({ unlockedCount: projectionCampaign.unlockedCount }).from(projectionCampaign).where(eq(projectionCampaign.chainCampaignId, chainCampaignId));
        expect(projection?.unlockedCount).toBe(1);
        const unlockJobs = await db.select({ status: relayerJob.status }).from(relayerJob).where(like(relayerJob.idempotencyKey, `voucher_unlock:${chainCampaignId}:${consumerAddress.toLowerCase()}%`));
        expect(unlockJobs).toHaveLength(1);
    });

    it("pays the café owner exactly one voucherPayout on redemption", async () => {
        const before = await chain.readContract({ address: addresses.mockPEN, abi: abis.mockPEN, functionName: "balanceOf", args: [ownerAddress as `0x${string}`] });
        const requested = await requestVoucherRedemptionService(consumerId, cafeId, { voucherId });
        expect(requested.ok, requested.ok ? undefined : JSON.stringify(requested.error)).toBe(true);
        if (!requested.ok) throw new Error("voucher redemption request failed");
        redemptionRequestId = requested.data.id;
        const approved = await decideVoucherRedemptionService(ownerId, cafeId, redemptionRequestId, { decision: "approved" });
        expect(approved.ok, approved.ok ? undefined : JSON.stringify(approved.error)).toBe(true);
        await drainRelayerAndIndexer();
        const after = await chain.readContract({ address: addresses.mockPEN, abi: abis.mockPEN, functionName: "balanceOf", args: [ownerAddress as `0x${string}`] });
        const [projected] = await db.select().from(projectionCampaign).where(eq(projectionCampaign.chainCampaignId, chainCampaignId));
    ...
    afterAll(async () => {
        ...
        if (redemptionRequestId) await db.delete(relayerJob).where(sql`${relayerJob.payload}->>'redemptionRequestId' = ${redemptionRequestId}`);
