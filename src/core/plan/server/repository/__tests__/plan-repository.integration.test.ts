...
async function fixture() {
    ...
    const userId = `plan-repository-user-${suffix}`;
    const cafeId = `plan-repository-cafe-${suffix}`;
    const chainCafeId = Number.parseInt(suffix.slice(0, 7), 16);
    await db.insert(user).values({
        id: userId,
        name: "Plan Repository User",
        email: `plan-repository-${suffix}@example.test`,
    });
    await db.insert(cafe).values({
        id: cafeId,
        name: "Plan Repository Cafe",
        slug: `plan-repository-${suffix}`,
        chainCafeId,
    });
    createdUserIds.push(userId);
    createdCafeIds.push(cafeId);
    return { userId, cafeId, chainCafeId };
}
...
describeIntegration("plan repository", () => {
    afterEach(async () => {
        ...
        for (const id of createdCafeIds.splice(0)) {
    ...
    it("never returns a submitted order to pending for retry", async () => {
        ...
        expect((await findInFlightByCafe(base.cafeId))?.status).toBe(
            "submitted",
        );
    });
    ...
    it("starts a fresh receipt-attempt budget when submission begins", async () => {
        const base = await fixture();
        const order = await insertOrderIfIdle(newOrder(base));
        await markOrderRetry(order.row.id, "rpc unavailable", 4, new Date());
